...
def yolo_face_detection(image_file_path, yolo_path, weights_file, classes_file, save_path):
    """
    ...
    # else if it's an mp4 video process the video
        ...
        video_stream = FileVideoStream(image_file_path).start()
        cam = cv2.VideoCapture(image_file_path)
        fps = cam.get(cv2.CAP_PROP_FPS)
        cam.release()

        # strip file extension of original image so we can write similar output image
        # i.e.) people.mp4 --> people_output.mp4
        video_file_name = os.path.splitext(image_file_path)[0]
